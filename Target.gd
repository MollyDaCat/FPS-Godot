...
func bullet_hit(damage, bullet_transform):
	...
	if current_health <= 0:
		...
		broken_target_holder.add_child(destroyed_target.instance())
		
		for rigid in destroyed_target.instance().get_children():
			if rigid is RigidBody:
				...
				#apply additional force to impulse
				rigid.apply_impulse(center_in_rigid_space, direction * 99 * damage)
